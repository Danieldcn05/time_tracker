...
class App(Base):
    ...
    ps_name = Column(String)  # Process name in the operating system
    tracking = Column(Boolean, default=False)  
    total_usage_time = Column(Integer, default=0)  # Total usage time in seconds
...
def create_log(session, app_id):
    ...
    app_name = get_app_name(session, app_id)
...
def close_log(session, app_id):
    print(f"Attempting to close log for app_id: {app_id}")
    ...
    if log:
        ...
        if log.end_time is None:
            ...
            print(f"The log already has an end_time: {log.end_time}")
    else:
        print("No log found to close.")
    return None
...
def update_app_usage_time(session, app_id, usage_time):
    ...
    if app:
        ...
        print(f"Usage time for {app.name}: {app.total_usage_time} seconds. Last session: {usage_time} seconds.")
        session.commit()
        return app
    return None
...
def get_app_name(session, app_id):
    app = session.query(App).filter_by(id=app_id).first()
    if app:
        return app.name
    return None
...
def fetch_logs(session):
    ...
    return [(log.app.name, log.start_time, log.end_time, format_timedelta(int((log.end_time - log.start_time).total_seconds())) if log.end_time else None) for log in logs]
...
def fetch_processes(session): # Returns the names of ALL processes
    apps = session.query(App).all()
    return [app.ps_name for app in apps]

def fetch_tracked_ps(session):
